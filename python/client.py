...
class Client:
    ...
    def set_state(self, arg_list):
        ...
        self.board.end = self.ends - 1 - int(arg_list[1])
        self.board.turn = dc.official_to_ayumu_turn(int(arg_list[0]))
        return 0
    ...
    def set_stone(self, arg_list):
        ...
        for i in range(0, len(arg_list), 2):
            p = dc.official_to_ayumu_position((float(arg_list[i]), float(arg_list[i + 1])))
            self.board.stone[dc.N_STONES - 1 - (i / 2)] = p
        return 0
    ...
    def play(self, arg_list):
        ...
        mv = dc.TEE_SHOT_R
        omv = dc.ayumu_to_official_move(mv)
        mv_str = ' '.join(['BESTMOVE', str(float(omv[0])), str(float(omv[1])), str(int(omv[2]))])
        print(mv_str)
        send_msg(self.sock, mv_str)
        return 0
    
    def do_command(self, cmd, arg_list):
        ucmd = cmd.upper()
        if not cmd in self.action_dct:
            return -1
        for act in self.action_dct[cmd]:
