                ...
                if(node.size != 0 || node.parent != nullptr){ // 本当に何の情報もないとき以外
                    ...
                    while(1){ // 上位のノードを辿っていくループ
                        ...
                        if(ptnode->size > 0){ // 何も記録がないノードはとばす
                            ...
                            const fpn_t firstSize = FIRST_SIZE_LEAFCHILD * dw;
                            ...
                            while(1){
                                if(buf[i0].data() == ptnode->child[i1].mv.data()){ // 一致
                                    allSizeSum += ptnode->child[i1].size * dw;
                                    buf[i0].size += ptnode->child[i1].size * dw;
                                    buf[i0].eval_sum += ptnode->child[i1].eval_sum * dw;
                                    buf[i0].mu += ptnode->child[i1].mu * dw;
                    ...
                    for(int i = 0; i < NMoves; ++i){
                        ...
                        buf[i].weight = buf[i].mean() + K_UCB_LEAF * sqrt(2) * sqrt(logAllSize / buf[i].size);
