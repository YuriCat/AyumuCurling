...
double genExponentialRand(const double lambda){
    std::uniform_real_distribution<double> uni(0, 1);
    return -log(uni(dice)) / lambda;
}

double genGammaRand(const double k){
    std::uniform_real_distribution<double> uni(0, 1);
    double x, y, z;
    double u, v, w, b, c, e;
    if(k < 1){
        e = genExponentialRand(1);
        do{
            x = pow(uni(dice), 1 / k);
            y = pow(uni(dice), 1 / (1 - k));
        }while (x + y > 1);
        return (e * x / (x + y));
    }else{
        b = k - 1;
...
double genBetaRand(const double a, const double b){
    double r1 = genGammaRand(a);
    double r2 = genGammaRand(b);
    return r1 / (r1 + r2);
}
...
enum {
    ...
    STATE_FREEGUARD		= STATE_PLAYAREA << 1,		// フリーガードゾーン
...
void Friction( float friction, SUBGAMESTATE *pSubGameState )
{
    ...
    for( int i=0; i<pSubGameState->ShotNum; i++ ){
        if( pSubGameState->body[i] != nullptr ){
            ...
            /*if( pSubGameState->body[i]->GetAngularVelocity()!=0.0f ){
             if(  pow(pSubGameState->body[i]->GetAngularVelocity(),2)!=pow(STANDARD_ANGLE,2) ){
             std::cout<<"w : "<<pSubGameState->body[i]->GetAngularVelocity()<<","<<std::endl;
             }
             }*/
...
int MainLoop( SUBGAMESTATE *pSubGameState, b2World *world, float timeStep, float *pLoci, size_t ResLociSize, int LoopCount = -1 )
{
    ...
    for( iRet=0; iRet < LoopCount || LoopCount == -1; iRet++ ){
        ...
        /*
         全てのストーンが静止したら true を返す
         ストーンがリンクから出た場合にはそのストーンを除外する
         */
        for( int i=0; i<pSubGameState->ShotNum; i++ ){
            if( pSubGameState->body[i] != nullptr ){
                // ストーンがリンク外に出たら除外する
                vec = pSubGameState->body[i]->GetPosition();
                if( GetStoneState( vec.x, vec.y ) == 0 ){
...
int GetScore( GAMESTATE *pGameState )
{
    /*
     先手側の得点を基準にスコアを返す。
     先手が得点した場合には正の数を、後手が得点した場合には負の数を返す。
     */
...
// ショットの生成
//=====================================================================================
b2Vec2 CreateShot( float x, float y )			/* ショットの強さを摩擦力を考慮した値に直す */
{
    b2Vec2 Shot;
    float len;
...
int Simulation( GAMESTATE *pGameState, SHOTVEC Shot, float Rand = STONEINFO_SIZE, SHOTVEC *lpResShot = nullptr, int LoopCount = -1 )
{
    ...
    //std::default_random_engine engine(seed_gen() ^ rand());
    ...
    // 平均 0, 標準偏差をストーン半径で分布させる
    if(Rand != 0.0f){
        float dvx, dvy;
#define RULE_ERROR_BETA
    
#ifdef RULE_ERROR_BETA
        // 元々のショットの速度ベクトル
        const double ov = hypot(Shot.x, Shot.y);
        const double otheta = atan2(Shot.y, Shot.x);
        
        // デリバリーの技術的誤差
        // 速さ : 0 ~ Vmax でベータ分布
        // 角度 : -π/64 ~ π/64 でベータ分布
        constexpr double Vmax = 33.7419;
        constexpr double Thmax = M_PI / 64;
        constexpr double angleStability = 50;
        constexpr double speedStability = 60;
        
        constexpr double DVmin = 3.0;
        
        std::cerr << ov << " " << otheta << std::endl;
        
        double vRate = DVmin / (Vmax - ov + DVmin);
        double nv = ov - DVmin + (Vmax - ov + DVmin) * genBetaRand(vRate * speedStability, (1 - vRate) * speedStability);
        double ntheta = otheta + 2 * Thmax * genBetaRand(angleStability, angleStability) - Thmax;
        
        // 不完全情報+スウィーピング誤差
        // 速さ : 正規分布
        // 角度 : ↑で一緒に扱う
        const double errorVSigma = Rand * 2;
        
        std::normal_distribution<float> distV(0, errorVSigma);
        
        nv += distV(dice);
        
        dvx = nv * cos(ntheta) - Shot.x;
        dvy = nv * sin(ntheta) - Shot.y;
        
        std::cerr << nv << " " << ntheta << std::endl;
        
#else
#ifdef RULE_ERROR_GAT
        // GAT用にx方向の乱数を半分に変更 2015/12/04
        //        y方向の乱数を倍に変更   2015/12/08
        std::normal_distribution<float> distX(0, Rand / 2);
        std::normal_distribution<float> distY(0, Rand * 2);
#else
        // 元々のルール
        std::normal_distribution<float> distX(0, Rand);
        std::normal_distribution<float> distY(0, Rand);
#endif
        ...
        //ShotPos.x = 2.375f + dist(engine);
        //ShotPos.y = 4.88f + dist(engine);
        // GAT用にx方向の乱数を半分に変更 2015/12/04
        //        y方向の乱数を倍に変更   2015/12/08
        //ShotPos.x = 2.375f + distX(engine);
        ShotPos.x = 2.375f + distX(dice);
        //ShotPos.y = 4.88f + distY(engine);
        ...
        // 誤差
        dvx = TeeShot.x - ShotVec.x;
        dvy = TeeShot.y - ShotVec.y;
#endif
        
        // ショットのセット
        if( SetShot( Shot, &SubGameState, &world, dvx, dvy, &WorkVec ) == false ){
            return false;
        }
    }
    else{
...
int SimulationEx( GAMESTATE *pGameState, SHOTVEC Shot, float RandX, float RandY, SHOTVEC *lpResShot, float *pLoci, int ResLociSize )
{
    ...
    if(RandX != 0.0f){
        std::normal_distribution<float> distX(0, RandX);
        //ShotPos.x += distX(engine);
        ShotPos.x += distX(dice);
    }
    if(RandY != 0.0f){
        std::normal_distribution<float> distY(0, RandY);
        //ShotPos.y += distY(engine);
